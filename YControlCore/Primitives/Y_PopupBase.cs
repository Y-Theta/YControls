...
namespace YControlCore.Primitives {
    ...
    public class Y_PopupBase : Popup {
        #region Properties
        /// <summary>
        /// 子控件窗口句柄
        /// </summary>
        ...
        /// 关闭popup
        /// </summary>
        public static CommandBase CloseCommand { get; set; }

        /// <summary>
        /// 所有的消息类Y_PopupBase实例
        /// </summary>
        private static List<Y_PopupBase> _msgPopups { get; set; }
        ...
        public static readonly DependencyProperty RelativeRectProperty =
            DependencyProperty.Register("RelativeRect", typeof(Point),
                typeof(Y_PopupBase), new FrameworkPropertyMetadata(new Point(0, 0),
        ...
        public static readonly DependencyProperty RelativeModeProperty =
            DependencyProperty.Register("RelativeMode", typeof(PopupRelativeMode),
                typeof(Y_PopupBase), new FrameworkPropertyMetadata(PopupRelativeMode.LeftTop,
                    FrameworkPropertyMetadataOptions.Inherits,
        ...
        public static readonly DependencyProperty AutoHideProperty =
            DependencyProperty.Register("AutoHide", typeof(bool),
                typeof(Y_PopupBase), new FrameworkPropertyMetadata(false,
                    FrameworkPropertyMetadataOptions.Inherits,
                    OnAutoHideChanged));
        private static void OnAutoHideChanged(DependencyObject d, DependencyPropertyChangedEventArgs e) {
            if ((bool)e.NewValue)
                ((Y_PopupBase)d).InitAutoHide();
            else
                ((Y_PopupBase)d).DisposeAutoHide();
        }
        ...
        public static readonly DependencyProperty AutoHideDelayProperty =
            DependencyProperty.Register("AutoHideDelay", typeof(int),
                typeof(Y_PopupBase), new FrameworkPropertyMetadata(0,
                    FrameworkPropertyMetadataOptions.Inherits,
                    OnAutoHideDelayChanged));
        private static void OnAutoHideDelayChanged(DependencyObject d, DependencyPropertyChangedEventArgs e) {
            if (((Y_PopupBase)d).AutoHide)
                ((Y_PopupBase)d)._autohide.Interval = (int)e.NewValue;
        }
        ...
        private static void OnEnableBlurChanged(DependencyObject d, DependencyPropertyChangedEventArgs e) {
            ((Y_PopupBase)d).SetBlur((bool)e.NewValue);
        }
        ...
        private void SetBlur(bool enable) {
            ...
            //是否启用Aero玻璃效果
            if (enable)
                BlurEffect.EnableBlur(GetVisualHandle(Child), AccentState.ACCENT_ENABLE_BLURBEHIND);
            else
                BlurEffect.EnableBlur(GetVisualHandle(Child), AccentState.ACCENT_DISABLED);
        }
        ...
        private void AddAttachWindowHook() {
            _holder.Activated += Y_PopupBase_Activated;
            _holder.Deactivated += Y_PopupBase_Deactivated;
            _holder.LocationChanged += (s, e) => UpdateLocation();
            _holder.SizeChanged += Y_PopupBase_SizeChanged;
            _holder.IsVisibleChanged += _holder_IsVisibleChanged;
            ((FrameworkElement)Child).SizeChanged += Y_PopupBase_SizeChanged;
        ...
        private void Y_PopupBase_Deactivated(object sender, EventArgs e) {
            if (_topmostparent) {
                if (_passiveActive) {
        ...
        private void Y_PopupBase_Activated(object sender, EventArgs e) {
        ...
        /// </summary>
        private void Y_PopupBase_SizeChanged(object sender, SizeChangedEventArgs e) {
            _update = true;
            ComputeOffset();
        ...
        static Y_PopupBase() {
            DefaultStyleKeyProperty.OverrideMetadata(typeof(Y_PopupBase), new FrameworkPropertyMetadata(typeof(Y_PopupBase),
                FrameworkPropertyMetadataOptions.Inherits));

            CloseCommand = CommandBase.New(obj=> { });
        }
        #endregion
    }
}
